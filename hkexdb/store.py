...
EXTRACTOR_VERSION = "2026-09-G"   # 2026-09-G：中文日期里的「三十日」不再当窗口；配对不再让无日期的行冒充同一天
# 2026-09-F：去重那行少一对括号会整份炸掉；要约价的数量级闸改成筛子；「百萬」被拆开也读得回来
# 2026-09-E：原文把基准价的小数点印丢了，按同一张梯子的量级复位
# 2026-09-D：印刷体梯子一行行读；被提前的日期不再当窗口；协议＋要约的合计不当规模
# 2026-09-C：config 的退档表补上月度窗口；千分位里不再切出假要约价
# 2026-09-B：错位读法再放宽一点；评分配对先看「判定＝要约」
# 2026-09-A：PDF 文字层错位 —— 数字被提到词的前面也能读
# Z：03938 真原文验收打包加总；「已發行股份將為N股」
