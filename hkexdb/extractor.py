...
# 数字和它的词之间还可能夹着几个字：
#   「要約價格的價值比較0.30**要約價格為**每股要約股份港元」（02362 金川）
_PRICE_REVERSED = re.compile(
    # ⚠️ 前面不能是逗号：「102,373,241.88港元」里 241.88 前面是逗号，
    #    只挡 (?<![\d.]) 的话会从千分位中间切一刀，抽出个 241.88 当要约价
    #    （06877 实跑真的这样，要约价印成 241.88）。
    r"(?<![\d.,])([\d,]+\.\d{1,4})(?:(?!每股)[^0-9。；]){0,12}?"
    r"每股(?:要約)?股份(?:現金)?\s*港元")
...
#
# ⚠️ 括号必须半角全角都认：01835 瑞威写「連續五（5）個交易日」，用的是
# 全角括号，而我们**故意不把全角标点折成半角**（折了会把释义节切名字用的
# 那个全角逗号也折掉）。只认半角的话，5 日和 10 日两条都会掉进「收市价」。
# ⚠️ 天数**必须跟着量词「個」**，不能只认光秃秃的「三十」。
# 08232 CLASSIFIED GP 那条是普通散文，一点都不错位：
#     「(i) 較股份於二零二五年六月三十日（即最後交易日）…收市價
#       每股1.090港元溢價約20.9%」
# 「六月三十日」里的「三十」被当成了三十个交易日 —— 于是收市价那条
# 顶掉了真正的 30 日均价（1.070／23.1%），主值取到 20.9%。
# 中文日期里「三十日」「三十一日」到处都是，这不是个别现象。
#
# 「個」和数字之间可以夹一个括号数字（表格排版把「三十(30)個」拆开），
# 也可以只剩括号里那个（中文数字被劈到了别处）。
def _days_window(cn: str, num: str):
    return re.compile(rf"(?:{cn}|{num})\s*[(（]?\s*(?:{num})?\s*[)）]?\s*個")
...
_WINDOWS = [
    ...
    ("180d", _days_window("一百八十", "180")),
    ("120d", _days_window("一百二十", "120")),
    ("90d", _days_window("九十", "90")),
    ("60d", _days_window("六十", "60")),
    ("30d", _days_window("三十", "30")),
    ("10d", _days_window("十", "10")),
    ("5d", _days_window("五", "5")),
]
