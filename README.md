...
| 双击这个 | 干什么 |
|---|---|
| **一键更新.bat** | 更新到最新版。`data\` 和 `logs\` 不动 |

更新也可以直接在程序里点：「抓取」页右下角有个 **检查更新** 按钮，
效果一样，不需要那个 `.bat`。

> 杀软报毒？原来那个 `.bat` 会自己下载 zip 再覆盖文件，卡巴斯基把它
> 判成 `PDM:Trojan.Win32.Generic.nblk` 删掉过。现在下载和覆盖都由
> `hkexdb\updater.py` 做，`.bat` 只负责把它叫起来。
| **一键运行.bat** | 打开程序 |
...
```
一键更新.bat      双击：更新到最新版（data/ logs/ .venv/ 不动）
                  实际干活的是 hkexdb/updater.py，界面上也有按钮
一键运行.bat      双击：打开程序
